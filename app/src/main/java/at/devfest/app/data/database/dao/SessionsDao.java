...
public class SessionsDao {
    ...
    public Observable<List<at.devfest.app.data.app.model.Session>> getSessions() {
        return getSessions("SELECT * FROM " + Session.TABLE);
    }

    public Observable<at.devfest.app.data.app.model.Session> getSessionById(int id) {
        return getSessions("SELECT * FROM " + Session.TABLE + " WHERE " + Session.ID + "=?", Integer.toString(id)).flatMap(Observable::from).first();
    }

    public Observable<List<at.devfest.app.data.app.model.Session>> getSelectedSessions() {
    ...
    public Observable<List<at.devfest.app.data.app.model.Session>> getSelectedSessions(LocalDateTime slotTime) {
    ...
    public void toggleSelectedSessionState(at.devfest.app.data.app.model.Session session, boolean insert) {
    ...
    public void saveSessions(List<at.devfest.app.data.app.model.Session> toSave) {
        Preconditions.checkNotOnMainThread();

        BriteDatabase.Transaction transaction = database.newTransaction();
        try {
            database.delete(Session.TABLE, null);
            for (at.devfest.app.data.app.model.Session session : toSave) {
    ...
    public Observable<Boolean> isSelected(at.devfest.app.data.app.model.Session session) {
    ...
    private Observable<List<at.devfest.app.data.app.model.Session>> getSessions(String query, String... args) {
